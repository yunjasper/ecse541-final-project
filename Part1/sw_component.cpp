        ...
        for (i = 0; i < matrix_size; i++) // row number -- execs: matrix_size * loops, iters: matrix_size
        {
            ...
            for (j = 0; j < matrix_size; j++) // column number -- execs: matrix_size * matrix_size * loops, iters: matrix_size
            {
                ...
                // no timing penalty for these calculations because assume optimized at compile/design time
                    ...
                    if (matrix_size >= 6) // works up to at least 150
                    {
                        if (j == 0)
                            wait((7 * matrix_size - 1) * CLOCK_PERIOD); // don't make new request before hardware is done
                        else
                            wait((4 * matrix_size + 1) * CLOCK_PERIOD);
                    }
                    else if (matrix_size >= 3)
                    {
                        if (j == 0)
                            wait((8 * matrix_size + 3) * CLOCK_PERIOD); // don't make new request before hardware is done
                        else
                            wait((5 * matrix_size + 5) * CLOCK_PERIOD);
                    }
                    else if (matrix_size == 2)
                    {
                        if (j == 0)
                            wait((9 * matrix_size + 3) * CLOCK_PERIOD); // don't make new request before hardware is done
                        else
                            wait((5 * matrix_size + 8) * CLOCK_PERIOD);
                    }
                    else if (matrix_size == 1)
                        wait((22 * matrix_size) * CLOCK_PERIOD);
