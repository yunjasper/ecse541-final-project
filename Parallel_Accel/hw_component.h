...
class Hw_component : public sc_module
{
    public:
        ...
        vector<double> reg_Aik_HW0;
        vector<double> reg_Aik_HW1;
        double reg_Lij;
        vector<double> reg_Lkj_HW0;
        vector<double> reg_Lkj_HW1;
        ...
        unsigned int data_len, data_len_HW0, data_len_HW1;

        // constructor
        Hw_component(sc_module_name name);
        void do_hw_component();
        void hw_master_read_data(unsigned int addr, vector<double>& reg, unsigned int transaction_len, unsigned int indexing_mode = ROW_MJR);
        void hw_master_read_data(unsigned int addr, double& reg, unsigned int indexing_mode = ROW_MJR);
        void hw_master_write_data(unsigned int addr, vector<double>& reg, unsigned int transaction_len); // write array
