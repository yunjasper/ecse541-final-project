...
Hw_component::Hw_component(sc_module_name name) : sc_module(name)
{
    ...
    {
        reg_Aik_HW0.push_back(0.0);
        reg_Aik_HW1.push_back(0.0);
        reg_Lkj_HW0.push_back(0.0);
        reg_Lkj_HW1.push_back(0.0);
    }
    reg_Lij = 0.0;
...
void Hw_component::do_hw_component()
{
    while (true)
    {
        ...
        if (req_addr == ADDR_MEM_HW)// && req_op == OP_CALC)
        {
            ...
            // hardware_cycles += CYCLES_HW_DIV;  // don't think we need to count this
            data_len_HW0 = data_len / 2;
            // cout << data_len_HW0 << endl;

            // hardware_cycles += CYCLES_SW_ADD;  // don't think we need to count this either
            data_len_HW1 = data_len - data_len_HW0;
            // cout << data_len_HW1 << endl;
            ...
            if(data_len_HW0 != 0){
                // hardware_cycles += CYCLES_HW_ADD * 4 + CYCLES_HW_MUL;  // "free" because parallel
                hw_master_read_data(base_mem_addr_loop + addrA + row_number * matrix_size + (col_number + 1), reg_Aik_HW0,  data_len_HW0, ROW_MJR); // read A_ik
            }
            hardware_cycles += CYCLES_HW_ADD * 5 + CYCLES_HW_MUL;
            hw_master_read_data(base_mem_addr_loop + addrA + row_number * matrix_size + (col_number + 1) + data_len_HW0, reg_Aik_HW1, data_len_HW1); // read A_ik
            ...
            if(data_len_HW0 != 0){
                // hardware_cycles += CYCLES_HW_ADD * 4 + CYCLES_HW_MUL;  // "free" because parallel
                hw_master_read_data(base_mem_addr_loop + addrL + (col_number + 1) * matrix_size + col_number, reg_Lkj_HW0, data_len_HW0, COL_MJR); // read L_kj
            }
            hardware_cycles += CYCLES_HW_ADD * 5 + CYCLES_HW_MUL;
            hw_master_read_data(base_mem_addr_loop + addrL + (col_number + 1 + data_len_HW0) * matrix_size + col_number, reg_Lkj_HW1, data_len_HW1, COL_MJR); // read L_kj
            ...
            for(int i = 0; i < data_len_HW1; i++){
                ...
                if(data_len_HW0 != 0){
                    reg_Aik_HW0[i] -= reg_Lij * reg_Lkj_HW0[i];
                }
                reg_Aik_HW1[i] -= reg_Lij * reg_Lkj_HW1[i]; // this transaction is "free" in our model since it's done in parallel
            }

            // 3. write finished data (row * column) to memory. matrix C is stored as row major
            if(data_len_HW0 != 0){
                // hardware_cycles += CYCLES_HW_ADD * 4 + CYCLES_HW_MUL;  // "free" because parallel
                hw_master_write_data(base_mem_addr_loop + addrA + row_number * matrix_size + (col_number + 1), reg_Aik_HW0, data_len_HW0);
            }
            hardware_cycles += CYCLES_HW_ADD * 5 + CYCLES_HW_MUL;
            hw_master_write_data(base_mem_addr_loop + addrA + row_number * matrix_size + (col_number + 1) + data_len_HW0, reg_Aik_HW1, data_len_HW1);
 ...
 */
void Hw_component::hw_master_read_data(unsigned int addr, vector<double>& reg, unsigned int transaction_len, unsigned int indexing_mode)
{
    // request and wait
    #ifdef DEBUG_HW
    debug_log_file << "[Hw_component] hw_master_read_data() : Requesting : MST_ID = " << MST_ID_HW << ", ADDR = " << addr <<", OP = OP_READ, len = " << matrix_size << endl;
    #endif
    if_bus_master->Request(MST_ID_HW, addr, OP_READ, transaction_len, indexing_mode);
    ...
    hardware_cycles += CYCLES_HW_ADD; // i = 0
    for (unsigned int i = 0; i < transaction_len; i++ )
    {
 ...
 */
void Hw_component::hw_master_write_data(unsigned int addr, vector<double>& reg, unsigned int transaction_len)
{
    // request and wait
    #ifdef DEBUG_HW
    debug_log_file << "[Hw_component] hw_master_write_data() : Requesting : MST_ID = " << MST_ID_HW << ", ADDR = " << addr <<", OP = OP_WRITE, len = " << matrix_size << endl;
    #endif
    if_bus_master->Request(MST_ID_HW, addr, OP_WRITE, transaction_len);
